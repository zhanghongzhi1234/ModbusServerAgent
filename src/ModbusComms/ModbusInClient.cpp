...
namespace TA_Base_Bus
{
	...
	bool ModbusInClient::readData(char* buf, const unsigned int length)
	{
		...
		while( !m_toTerminate ) 
		{
			...
			if (0 == result)
			{
				LOG_GENERIC(SourceInfo, DebugUtil::DebugError, "ModbusInClient receive EOF, recv_n length %d, bytesRead %d, return %d", length, bytesRead ,result);
				return false;
			}
	...
	bool ModbusInClient::readInputRegisters(char& eCode)
	{
		...
		printf("readInputRegisters - addr %d, length %d: m_startWord %d, m_length %d\r\n", addr, length, m_startWord, m_length);
